...
public class BlockMetamorphicTable extends Block implements ITileEntityProvider {
	...
	public BlockMetamorphicTable() {
		...
		this.setHardness(10);
		ACTIVATION_BLOCK = Blocks.ENCHANTING_TABLE;//Block.getBlockFromName(ConfigHandler.metamorphicTableActivationBlocks);
		ACTIVATION_STATE = 0;//ConfigHandler.metamorphicTableActivationMeta;
	}
	...
	public void trailParticles(World world, BlockPos pos, Random rand) {
		...
		// world.spawnParticle(EnumParticleTypes.DRAGON_BREATH, north.getX()+0.5,
		// north.getY()+0.15, north.getZ()+0.5, -0.125, -0.0075, 0, new int[0]);
		// world.spawnParticle(EnumParticleTypes.DRAGON_BREATH, east.getX()+0.5,
		// east.getY()+0.15, east.getZ()+0.5, 0, -0.0075, -0.125, new int[0]);
		// world.spawnParticle(EnumParticleTypes.DRAGON_BREATH, south.getX()+0.5,
		// south.getY()+0.15, south.getZ()+0.5, 0.125, -0.0075, 0, new int[0]);
		// world.spawnParticle(EnumParticleTypes.DRAGON_BREATH, west.getX()+0.5,
		// west.getY()+0.15, west.getZ()+0.5, 0, -0.0075, 0.125, new int[0]);

		for (int i = 0; i < 5; i++) {
			Underworld.proxy.spawnCustomParticle("firefly", world, north.getX(), north.getY() + 0.15, north.getZ(), -25, -25, 0, 10, 0.5f, 0, 0.5f, false);
			Underworld.proxy.spawnCustomParticle("firefly", world, east.getX(), east.getY() + 0.15, east.getZ(), 0, -25, -25, 10, 0.5f, 0, 0.5f, false);
			Underworld.proxy.spawnCustomParticle("firefly", world, south.getX(), south.getY() + 0.15, south.getZ(), 25, -25, 0, 10, 0.5f, 0, 0.5f, false);
			Underworld.proxy.spawnCustomParticle("firefly", world, west.getX(), west.getY() + 0.15, west.getZ(), 0, -25, 25, 10, 0.5f, 0, 0.5f, false);
		}
	}
	...
	public void tryToActivate(World world, BlockPos pos, EntityPlayer player, TileMetamorphicTable table) {
		...
		if (world.getBlockState(pos.north(3)) == ACTIVATION_BLOCK.getStateFromMeta(ACTIVATION_STATE) && world.getBlockState(pos.east(3)) == ACTIVATION_BLOCK.getStateFromMeta(ACTIVATION_STATE) && world.getBlockState(pos.south(3)) == ACTIVATION_BLOCK.getStateFromMeta(ACTIVATION_STATE) && world.getBlockState(pos.west(3)) == ACTIVATION_BLOCK.getStateFromMeta(ACTIVATION_STATE)) {
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		if (!world.isRemote && !isBlocked(world, pos)) {
			...
			if (!player.getHeldItem(hand).isEmpty() && player.getHeldItem(hand).getItem() == Items.BOOK) {
				table.renderMultiblock = true;
			}

			if (!table.isActivated && player.getHeldItem(hand).isEmpty()) {
			...
			if (!handStack.isEmpty() && table.inventory.getStackInSlot(0).isEmpty()) {

				table.inventory.insertItem(0, handStack, false);
				if (!table.hasValidRecipe()) {
					table.inventory.extractItem(0, table.inventory.getStackInSlot(0).getCount(), false);
					return true;
				}

				ItemStack left = new ItemStack(handStack.getItem(), handStack.getCount() - 1, handStack.getItemDamage());
