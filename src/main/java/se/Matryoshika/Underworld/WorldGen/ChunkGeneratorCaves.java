...
public class ChunkGeneratorCaves implements IChunkGenerator {

	// private static final String JSON;

	public final IBlockState STRATUM_HIGH;
	public final IBlockState STRATUM_MID;
	public final IBlockState STRATUM_LOW;
	public final IBlockState BEDROCK;
	...
	private final ChunkGeneratorSettingsCaves settings;
	
	private MapGenBase caveGenerator = new MapGenCaves();

	public ChunkGeneratorCaves(World world, long seed, boolean mapFeaturesEnabledIn, String generatorOptions) {
		
		{
			//caveGenerator = net.minecraftforge.event.terraingen.TerrainGen.getModdedMapGen(caveGenerator, net.minecraftforge.event.terraingen.InitMapGenEvent.EventType.CAVE);
		}

		this.STRATUM_HIGH = Blocks.STONE.getDefaultState();
		this.STRATUM_MID = Blocks.BRICK_BLOCK.getDefaultState();
		this.STRATUM_LOW = Blocks.STONEBRICK.getDefaultState();
		this.BEDROCK = Blocks.BEDROCK.getDefaultState();
		this.world = world;
		this.seed = seed;
		...
		ContextUnderworld ctx = new ContextUnderworld(minLimitPerlinNoise, maxLimitPerlinNoise, mainPerlinNoise, surfaceNoise, scaleNoise, depthNoise);
		...
		this.settings = ChunkGeneratorSettingsCaves.Factory.jsonToFactory(generatorOptions).build();

		this.settings.seaLevel = WorldProviderCaves.STRATUM_HIGH;
		this.settings.heightScale *= 1.5;
		this.settings.stretchY = 12;
		this.settings.mainNoiseScaleY *= 2;
		this.settings.depthNoiseScaleX *= 2;
		this.settings.depthNoiseScaleZ *= 2;
	...
	public void setBlocksInChunk(int chunkx, int chunkz, ChunkPrimer primer) {
						...
						for (int k2 = 0; k2 < 4; ++k2) {
							...
							for (int l2 = 0; l2 < 4; ++l2) {
								if ((lvt_45_1_ += d16) > 0.0D) {
									primer.setBlockState(i * 4 + k2, i2 * 8 + j2 + 126, l * 4 + l2, STRATUM_HIGH);
									primer.setBlockState(i * 4 + k2, i2 * 8 + j2 + 63, l * 4 + l2, STRATUM_MID);
									primer.setBlockState(i * 4 + k2, i2 * 8 + j2, l * 4 + l2, STRATUM_LOW);
								}
		...
		for(int bx = 0; bx < 16; bx++) {
			for(int bz = 0; bz < 16; bz++) {
				primer.setBlockState(bx, 0, bz, BEDROCK);
			}
		}
	}
			...
			for (int l = 0; l < 5; ++l) {
				...
				for (int j1 = -2; j1 <= 2; ++j1) {
					for (int k1 = -2; k1 <= 2; ++k1) {
						...
						if (this.world.getWorldType() == WorldType.AMPLIFIED && f5 > 0.0F) {
							f5 = 1.0F + f5 * 2.0F;
							f6 = 1.0F + f6 * 4.0F;
						}
